...
def validate_model(model: str, model_to_use: str, supported_models: set) -> None:
    if model and model_to_use not in supported_models:
        raise ValidationError(
            f"Model '{model_to_use}' is not supported. Use one of: {sorted(supported_models)}"
        )

def validate_api_key(api_key: str) -> None:
    if not api_key:
        raise ValidationError(
            "API key is required. Please provide your DashScope API key."
        )
